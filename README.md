# CSE Learning Journey 🚀

**Goal:** Get 15 LPA package after graduation

**Current Status:** Year 1 CSE Student | Just Started

---

## About Me
- **Name:** Harshatha
- **College:** [Your College Name]
- **Branch:** CSE (Computer Science Engineering)
- **Goal:** 15 LPA in placement
- **Learning:** Full Stack Development + DSA

---

## My Learning Path

### Phase 1: Fundamentals (Current - Months 1-3)
- [ ] Master Data Structures
- [ ] Learn Core Programming (Python/Java)
- [ ] Practice LeetCode Easy Problems
- [ ] Build first project
- [ ] Target CGPA: 8.0+

### Phase 2: Intermediate (Months 4-8)
- [ ] Solve Medium LeetCode Problems
- [ ] First Internship
- [ ] Build 2-3 real projects
- [ ] Learn Web Development

### Phase 3: Advanced (Months 9-36)
- [ ] Solve Hard LeetCode Problems
- [ ] System Design Basics
- [ ] 2nd Internship
- [ ] Specialize (Web/ML/Cloud)

### Phase 4: Placement Ready (Months 37-48)
- [ ] Mock Interviews
- [ ] Final Projects
- [ ] Campus Placements
- [ ] Target: 15+ LPA 🎯

---

## Projects

### Project 1: [Coming Soon]
- Description
- Technologies: 
- Link: [GitHub Link]

### Project 2: [Coming Soon]
- Description
- Technologies:
- Link: [GitHub Link]

---

## LeetCode Progress
- **Easy Problems Solved:** 0/50
- **Medium Problems Solved:** 0/30
- **Hard Problems Solved:** 0/10
- **Current Streak:** 0 days

[Track on LeetCode](https://leetcode.com/harshethaa08)

---

## Skills

### Languages
- Python (Learning)
- Java (Learning)

### Web Development
- HTML (Learning)
- CSS (Learning)
- JavaScript (Coming)

### Core Concepts
- Data Structures (In Progress)
- Algorithms (In Progress)
- DBMS (In Progress)

---

## Internships
- **Summer 2026:** (Coming)
- **Summer 2027:** (Coming)

---

## Resources I'm Using
- LeetCode
- GeeksforGeeks
- YouTube (Kunal Kushwaha, NeetCode)
- Coursera

---

## Daily Routine
- **2 hours:** Course studies
- **1-2 hours:** LeetCode coding
- **30 mins:** Project work

---

## Contact
- **GitHub:** [harshethaa08](https://github.com/harshethaa08)
- **LinkedIn:** [Your LinkedIn]
- **Email:** [Your Email]

---

## Progress Updates

**Week 1 (Current):**
- Created GitHub profile ✓
- Created LeetCode account ✓
- Learning Python basics (In progress)

---

**Last Updated:** may 2026

*"Every day I code, every day I get closer to 15 LPA" 💪*
